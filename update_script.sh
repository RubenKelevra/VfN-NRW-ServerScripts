...
if [ "$localsum" == "$remotesum" ]; then
    ...
    ssh -o ServerAliveInterval=2 -o ServerAliveCountMax=5 -o ConnectTimeout=20 root@$HostIP "rm /tmp/ff-stats/* ; rm tmp/wlanscan ; echo 3 > /proc/sys/vm/drop_caches; sleep 2 ; sysupgrade -n /tmp/$Filename"
else
